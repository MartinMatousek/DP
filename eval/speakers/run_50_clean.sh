...
CUDA_VISIBLE_DEVICES=$id $python_path eval_50_clean.py \
	--speakers 1 \
	--task $task \
	--test_dir $test_dir \
...
# mkdir -p $expdir && echo $uuid >> $expdir/run_uuid.txt
# echo "#esults from the following experiment will be stored in $expdir"


# echo "Stage 2/4 : S2 clean"
# CUDA_VISI#BLE_DEVICES=$id $python_path eval_50_clean.py \
	# --task $task \
	# --test_dir $test_dir \
	# --use_gpu $eval_use_gpu \
	# --exp_dir ${expdir} | tee logs/eval_${tag}.log
